...
class M_BAP_Asprak_Kehadiran extends Model
{
  ...
  {
    $this->select('id_kehadiran, nama_asprak, date_format(jam_masuk, "%Y-%m-%d") tanggal, date_format(jam_masuk, "%H:%i") masuk, date_format(jam_keluar, "%H:%i") keluar, jumlah_jam, kelas, matakuliah.kode_mk, matakuliah.nama_mk, modul_praktikum, approve_dosen');
    $this->join('asprak_list', 'bap_asprak_kehadiran.id_asprak_list = asprak_list.id_asprak_list');
    ...
    $this->where('bap_asprak_kehadiran.kode_dosen', $kode_dosen);
    $this->orderBy('approve_dosen', 'ASC');
    $this->orderBy('jam_masuk', 'DESC');
    $this->orderBy('id_kehadiran', 'DESC');
    return $this->findAll();
  ...
  public function approveKehadiran($id)
  {
    ...
    $this->where('substr(sha1(id_kehadiran), 13, 7)', $id);
    $this->update();
  }
